...
1. **설치** — 공식 사이트 [hermes-agent.nousresearch.com](https://hermes-agent.nousresearch.com)의 **Download for Windows**로 설치 파일을 받아 설치합니다(터미널을 선호하면 같은 페이지의 `irm https://hermes-agent.nousresearch.com/install.ps1 | iex`). 첫 실행에서 모델 제공자를 물으면 **"provider later"(나중에 선택)** 를 고릅니다. 노트북에는 API 키를 심지 않습니다 — 추론은 VPS가 합니다.
...
> **Windows 11 기동 충돌 이력(해결됨).** 초기 빌드는 Windows 11에서 실행 즉시 렌더러·GPU 자식 프로세스가 동일 offset의 `STATUS_BREAKPOINT`로 종료되는 문제가 있었습니다([NousResearch/hermes-agent#38216](https://github.com/NousResearch/hermes-agent/issues/38216)). 이 이슈는 이후 수정되어 **닫혔고**(PR #66842), 수정이 포함된 빌드에서는 Windows 11에서 정상 설치·연결됩니다. 옛 빌드에서 기동 즉시 충돌하면 공식 페이지에서 **현재 빌드를 다시 받으십시오** — 며칠 사이 배포본이 갱신되어 같은 Windows 11에서 실패하던 설치가 성공하는 것을 실제로 확인했습니다. 어느 경우든 **Web Dashboard(브라우저)** 는 데스크톱 없이도 같은 관리 기능을 제공하는 대안입니다.
